...
/**
 * Fast Parallel Game of Life for ImageJ (ByteProcessor version) - Replaces
 * ShortProcessor/short[] with ByteProcessor/byte[] (0 or 255 values) - One-loop
 * double-buffering approach and parallel per-row processing
 */
		...
		for (int it = 0; it < n; it++) {
			...
			IntStream.range(0, HEIGHT).parallel().forEach(y -> {
				...
				for (int x = 0; x < WIDTH; x++) {
					...
					int neighbors = (currentGen[x_m[x] + ym] & 0xff) + (currentGen[x + ym] & 0xff)
							+ (currentGen[x_p[x] + ym] & 0xff) + (currentGen[x_m[x] + y_offset] & 0xff)
							+ (currentGen[x_p[x] + y_offset] & 0xff) + (currentGen[x_m[x] + yp] & 0xff)
							+ (currentGen[x + yp] & 0xff) + (currentGen[x_p[x] + yp] & 0xff);
